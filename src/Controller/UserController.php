...
class UserController extends Controller {
	 ...
	 */
	public function byRole(RepositoryFinder $repoFinder, $role) {
		$roles = $role ? [User::normalizeRoleName($role)] : User::ROLES;
		$users = $repoFinder->forUser()->findUsersWithRole($role);
	 ...
	 */
	public function showBookRevisions(Request $request, RepositoryFinder $repoFinder, User $user) {
		$pager = $this->pager($request, $repoFinder->forBook()->revisionsFromUser($user), 30);
